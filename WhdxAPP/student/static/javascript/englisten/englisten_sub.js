...
function GetAllQuestion(){
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if(codenum==0){
                javascript:bc.musicUrl(AllData.paper.url);
...
function CreatInOrder(data){
    ...
    for(var i=0;i<data.length;i++){
        if(data[i].groupCode==null){
            ...
            $Com_NavList+=OptionClolor(result,i);
        }
        else {
            //不可拆分
...
//选项颜色
function OptionClolor(Result,i){
    var $Select='';
    if(Result==1){
        $Select+='<li class="Com_Nav r_OptionWrong" id="'+i+'"  data-index="'+i+'">'+(i+1)+'</li>';
    }else if(Result==0){
        $Select+='<li class="Com_Nav r_OptionRight" id="'+i+'"  data-index="'+i+'">'+(i+1)+'</li>';
    }else if(Result==2){
        $Select+='<li class="Com_Nav r_OptionHalf" id="'+i+'"  data-index="'+i+'">'+(i+1)+'</li>';
    } else {
        $Select+='<li class="Com_Nav r_OptionNo"  id="'+i+'" data-index="'+i+'">'+(i+1)+'</li>';
    }
    return $Select;
}
//题目操作
function Opration(){
    $('#r_MyAnswerUrl').on('click',function(){
        window.location.href="englisten_myanswer.html?id="+Request.id;
    });
...
function Answer(data){
    ...
    for(var k=0;k<data.labels.length;k++){
        ...
        if(data.labels[k].questionType=='03'||data.labels[k].questionType==03){
            // var answer=data.labels[k].content.replace(/答案/g,'答案: ');
            // answer=answer.replace("【","");
            // answer=answer.replace("】","");
            // answer=answer.replace(/答<\/span><span>案/g,'答案: ');
            // $AnalysisQue+='<div class="r_OpNomalColor">'+answer+'</div>';
        ...
        if(data.labels[k].questionType=='05'||data.labels[k].questionType==05){
            // var Analysis=data.labels[k].content.replace(/解析/g,'');
            // Analysis=Analysis.replace("【","");
            // Analysis=Analysis.replace("】","");
            // $AnalysisQue+='<div class="r_OpNomalColor">'+Analysis+'</div>';

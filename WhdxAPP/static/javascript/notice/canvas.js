/**
 * Created by lichao on 2016/12/30 0030.
 */
var Stroke = function () {
};
Stroke.prototype = {
    ...
    init: function (imgId, convasid, brushid, gobackid, markList) {
        ...
        this.touch = true;//("createTouch" in document);//判定是否为手持设备
    ...
    Bind: function () {
        ...
        this.canvas['on' + t.MoveEvent] = function (e) {
            ...
            {
                var touchX = touch.pageX;//鼠标在画布上的x坐标，以画布左上角为起点
                var touchY = touch.pageY;//鼠标在画布上的y坐标，以画布左上角为起点
                if (t.isbrush) {
    ...
    DrawImage: function (markList) {
        var t = this;
        t.images.src = "../../../static/image/notice/background.png";  //用户提交的作业图片路径
        t.images.onload = function () {
            t.cxt.drawImage(t.images, 0, 0);
        };
        if (markList != null) {
    ...
    cPush: function () {
        ...
        pro.flag = "2";
        console.log(t.cPushArray)
        if (imgArray.length > 0) {
            console.log(imgArray)
            var b = true;
            for (var i in imgArray) {
                console.log(imgArray[i].answerPicId);

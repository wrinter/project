...
getPaperDetail.prototype={
    ...
    assign_paper: function(){
        ...
        param.knowledgeId=this.getUrl().split("&")[2].split("=")[1];
        param.categoryId=this.getUrl().split("&")[3].split("=")[1];
        $.ajax({
            "type": "post",
                    ...
                    for(var _k in questionLines[k].questionGroup){
                        for(var key in questionLines[k].questionGroup[_k].questions){
                            if(questionLines[k].questionGroup[_k].questions[key].questionId==questionId){
                                ...
                                subject+="<div class='paperDetailTitle'>"+"<label>"+questionLines[k].questionGroup[_k].lnOrder+"、</label>"+questionTitle.replace("【题干】","").replace("<img","<img style='max-width:80%;' ")+"</div>";

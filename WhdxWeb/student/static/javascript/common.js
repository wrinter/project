...
$(document).ready(function(){
    function backtop(classname){$(classname).on("click",function(){$('body,html').animate({ scrollTop: 0 }, 500)})}
    backtop('.backtop');
});
/*******************************个人中心  弹出层的 显示 与隐藏**************************************/
...
function NoDataImg(type){
    ...
    if(type==0){
        ...
        $NoData+='<img src="../../static/image/kola/no.jpg" class="NoDataImg" alt="">';
    }
...
/*点击a淡rub封装*/
function FadeIn(a,b){$(a).on('click',function(){$(b).fadeIn(200);});}
/*点击a淡出b封装*/
function FadeOut(a,b){$(a).on('click',function(){$(b).fadeOut(200);});}
/****************************************阻止冒泡**********************************************/
...
var jiathis_config = {
    ...
    marginTop: 150,
    summary: "来自五好导学网",
    //pic: "static/image/login/logo.png",
...
function CheckTimeOut(){
    ...
    if(Href.indexOf('chinese_article.html')!=-1  || Href.indexOf('readtimereport.html')!=-1 || Href.indexOf('DH_EnglishLetters.html')!=-1 || Href.indexOf('DH_lettres.html')!=-1 || Href.indexOf('readreport.html')!=-1 ){
        return false;
    }else if(Href.indexOf('prepare_video_prepareVideoPlay.html')!=-1||Href.indexOf('IsShare=0#jtss-cqq')!=-1 || Href.indexOf('videoreport.html')!=-1 || Href.indexOf('DH_video.html')!=-1){
        store.set("vedioUrl",Href);
        return false;
    }
    if(Href.indexOf('acrossenglish_voice')!=-1 || Href.indexOf('DH_EnglishLetters')!=-1 || Href.indexOf('DH_lettres')!=-1 || Href.indexOf('readreport.html')!=-1 ){
        return false;
    }
    var Time=null;
    var Time1=null;
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.retCode == '9304'||data.retCode == 9304){
                $('#c_ErrorMsg').html('未登录，即将为您跳转首页').fadeIn(200); Disappear("#c_ErrorMsg");
            ...
            if (data.retCode == '0000'||data.retCode == 0000){
                $('#c_ErrorMsg').html('登录超时，即将跳转首页').fadeIn(200); Disappear("#c_ErrorMsg");
                Time1=setTimeout(function(){
                    Quit();
                    clearTimeout(Time1);
                },2000);
...
function SystemRedMsg(){
    ...
    var TestEng=(Href.indexOf('readtimereport.html')!=-1 );
    var TestLet=(Href.indexOf('readreport.html')!=-1 );
    ...
    if(Chinese||ChiTeacher||TestEng||TestLet||EntArt||EntVoice||Index||Found||Reg){
        return false;
    }else {
        $.ajax({
            "type": "post",
...
function GoldAnimate(GoldNum){
    ...
    $GoldHtml+='<img src="../../../student/static/image/common/goldimg.png" alt="" class="Com_GoldImg">';
    $GoldHtml+='<p class="Com_GoldNum" id="Com_GoldNum"></p>';
    $GoldHtml+='<audio src="../../../student/static/plugin/song/gold.wav" autoplay id="GoldAudio"></audio>';
    $GoldHtml+='</div>';

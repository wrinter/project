    ...
    studentPaper.prototype = {
        ...
        clickTime : function(testTime){
            ...
            function timer(){
                ...
                mm = checkTime(mm);
                ss = checkTime(ss);
                document.getElementById("model_paper_hour").innerHTML = hh;
                document.getElementById("model_paper_min").innerHTML = mm;
                document.getElementById("model_paper_second").innerHTML = ss;
                if( hh == 0 && mm == 0 && ss == 0) {
                    clearInterval(timer);
        ...
        getPaper : function(parmas){//获取试卷
            ...
            $.ajax({
                type : "post",
                url : "/web/student/modeltest/bulid",
                data : parmas,
                dataType : "json",
                    ...
                    if(data.retCode == "0000"){
                        ...
                        for(var q = 0 ; q < data.retData.resRandomPaperQuestion.length; q++) {
                            ...
                            for (var i = 0; i<resDtrue .resRandomPaperConfLn.length;i++) {
                                ...
                                //var re = _this.lowCase(Num+1)//大小写
                                ...
                                if(Dtrue.questions == null){//判断是否为空
                                }else{
                                    for(var j = 0;j<Dtrue.questions.length;j++){
                                        ...
                                        //选择题
                                        var $option = $("<div class='s_xpoption' selectableType='"+selectableType+"' id='"+id+"'  data_answer='"+answer+"' myscore='-0' style='position: relative'></div>");
        ...
        changeTest : function(){//换一套
            $(".model_paper_change").on("click",function(){   //换一套题
                if($(".model_paper_change").hasClass("add")){
                }else{
                    location.reload();
                }
            })
        ...
        shark : function(data){//提交时遮罩层
            var _this = this;
            $(".model_paper_delete_correr,.model_paper_success_correr,.model_correct_del,.model_correct_suc").on("click",function(){//取消
                $(".shark,.correct_sharks").hide();
            });
            $(".model_paper_false_correr,.model_correct_fal").on("click",function(){//确定
                $(this).closest(".shark").hide();
                //计时器
                if(parseInt($(".model_paper_hour").html()) != 0){
                    var hourmin = parseInt($(".model_paper_hour").html()*60);
                }
                var minmin = parseInt($(".model_paper_min").html());
                if(hourmin){
                    var time = parseInt(hourmin+minmin);
                }else{
                    var time = parseInt(minmin);
                }
                var time = Number(data.retData.testTime) - Number(time);
        ...
        sendPaper : function(data){//提交试卷
            ...
            s_paper_btton.on("click",function(){
                ...
                if(s_paper_btton.hasClass("show") && s_paper_btton.hasClass("display")){
                    ...
                    var time = Number(data.retData.testTime) - Number(time);//用时
                    sessionStorage.setItem("time",JSON.stringify(time));
                    _this.sendAjax(time);
                }else{
                    $(".shark").show();
        ...
        time : function(time){//时间问题显示
            if(time >= 60){
                var hour = parseInt(time%60);
                var min = parseInt(time/60);
                if(hour >= 10){
                    $(".model_paper_hour").text(hour);
                }else{
                    $(".model_paper_hour").text("0"+hour);
                }
                if(min >= 10){
                    $(".model_paper_min").text(min);
                }else{
                    $(".model_paper_min").text("0"+min);
                }
            }else{
                if(time >= 10){
                    $(".model_paper_min").text(time);
                }else{
                    $(".model_paper_min").text("0"+time);
                }
            }
        },
        sendAjax : function(time){//提交试卷ajax
            var _this = this;
            //客观题判断对错
            ...
            $.ajax({
                ...
                success : function(data){
                    if(data.retCode == "0000"){
                        ...
                        $(".model_paper_change,.correct_sharks").hide();
                        $(".wr").show();//显示对错
                        _this.enter();

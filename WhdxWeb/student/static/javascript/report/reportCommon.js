/**
 * Created by zxl on 2017/4/28.
 */
//圆环
function showCircle(className,percent){
    var num = percent *100* 3.6;
    if(num>180){
        $(className).find('.right').css('transform', "rotate(180deg)");
        $(className).find('.left').css('transform', "rotate(" + (num - 180) + "deg)");
    }else{
        $(className).find('.left').css('transform', "rotate(" + num + "deg)");
    }
...
function classTarget(id){
    var parmas = {};
    parmas.id = id;
    $(".r_classtarget").on("click", function () {
        $.ajax({
            type: "post",

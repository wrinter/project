...
function GetHomeWorkList(pageNum){
    ...
    $.ajax({
        "type": "post",
        "url": "/web/teacher/paper/report/testList",
        "dataType": "json",
        "data": SubData,
...
function Paging(PageSize){
    ...
    kkpager.generPageHtml({
        ...
        hrefFormer : 'testpaper_index', //链接尾部
        hrefLatter : '.html',
...
function CreatHomeWorkListHtml(data){
    if(data.length>0){
        ...
        for(var i=0;i<data.length;i++){
            ...
            $HomeWorkListHtml+='<p>'+data[i].assignTime+'</p>';//序号
            $HomeWorkListHtml+='<p>'+data[i].obj+'</p>';//布置对象
            //视频
            if(data[i].type=='300'){
            ...
            $HomeWorkListHtml+='<p>'+data[i].average+'</p>';//平均分
            $HomeWorkListHtml+='<p>';
            //视频
            if(data[i].type=='300'){
                $HomeWorkListHtml+='<a href="../../../model/testpaper/testpaper_VideoReport.html?paperAssignId='+data[i].id+'" class="w_Looka0">查看</a>';
                $HomeWorkListHtml+='<a href="../../../model/testpaper/testpaper_VideoReport.html?paperAssignId='+data[i].id+'" class="w_Looka1">查看</a>';
            }
            //文章
            else if(data[i].type=='401'||data[i].type=='402'||data[i].type=='403'){
                $HomeWorkListHtml+='<a href="../../../model/testpaper/testpaper_ArtReport.html?paperAssignId='+data[i].id+'" class="w_Looka0">查看</a>';
                $HomeWorkListHtml+='<a href="../../../model/testpaper/testpaper_ArtReport.html?paperAssignId='+data[i].id+'" class="w_Looka1">查看</a>';
            }
            //其他
            else{
                $HomeWorkListHtml+='<a href="../../../model/testpaper/testpaper_OtherReport.html?paperAssignId='+data[i].id+'&name='+data[i].name+'&avg='+data[i].average+'&type='+data[i].type+'" class="w_Looka0">查看</a>';
                $HomeWorkListHtml+='<a href="../../../model/testpaper/testpaper_OtherReport.html?paperAssignId='+data[i].id+'&name='+data[i].name+'&avg='+data[i].average+'&type='+data[i].type+'" class="w_Looka1">查看</a>';
            }
            $HomeWorkListHtml+='</p>';
            $HomeWorkListHtml+='</li>';
        }

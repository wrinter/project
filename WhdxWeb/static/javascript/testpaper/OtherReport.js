...
function HundredsHtml(avg){
    var Hundreds=avg.substr(0,1);
    var tens=avg.substr(1,1);
    var digit=avg.substr(2,1);
    var $HundredHtml='';
    $HundredHtml+='<img src="../../../static/image/Number/g'+Hundreds+'.png" alt="">';
    $HundredHtml+='<img src="../../../static/image/Number/g'+tens+'.png" alt="">';
    $HundredHtml+='<img src="../../../static/image/Number/g'+digit+'.png" alt="">';
    $('#Grade').html($HundredHtml);
}
function TensHtml(avg){
    var tens=avg.substr(0,1);
    var digit=avg.substr(1,1);
    var $HundredHtml='';
    $HundredHtml+='<img src="../../../static/image/Number/g'+tens+'.png" alt="">';
    $HundredHtml+='<img src="../../../static/image/Number/g'+digit+'.png" alt="">';
    $('#Grade').html($HundredHtml);
}
function DigitHtml(avg){
    var digit=avg.substr(0,1);
    var $HundredHtml='';
    $HundredHtml+='<img src="../../../static/image/Number/g'+digit+'.png" alt="">';
    $('#Grade').html($HundredHtml);
}
...
//下载
function Download(){
    var Url="/web/teacher/paper/report/studentTestDetailsDownload?paperAssignId="+Request.paperAssignId+'&type='+Request.type;
...
$('#w_PrintBtn0').attr('href','testPrint.html?paperAssignId='+Request.paperAssignId+'&name='+$('#w_OtherTitle').html()+'&type='+Request.type);
//学生详情
...
function CreatStudentDetailsHtml(data){
    ...
    for(var i=0;i<data.length;i++){
        ...
        $StudentDetailsHtml+='<p>'+(i+1)+'</p>';//序号
        $StudentDetailsHtml+='<p>'+data[i].studentName+'</p>';//姓名
        $StudentDetailsHtml+='<p>'+data[i].score+'</p>';//成绩
        ...
        if(data[i].used=='-1'||data[i].used==-1){
            ...
            $StudentDetailsHtml+='<a href="test_Subject.html?paperAssignId='+data[i].paperAssignId+'&name='+$('#w_OtherTitle').html()+'&type='+Request.type+'&time='+data[i].used+'&userid='+data[i].studentId+'&score='+data[i].score+'" class="w_Looka0" target="_blank">查看</a>';
            $StudentDetailsHtml+='<a href="test_Subject.html?paperAssignId='+data[i].paperAssignId+'&name='+$('#w_OtherTitle').html()+'&type='+Request.type+'&time='+data[i].used+'&userid='+data[i].studentId+'&score='+data[i].score+'" class="w_Looka1" target="_blank">查看</a>';
        }
        $StudentDetailsHtml+='</p>';
        $StudentDetailsHtml+='</li>';
    }

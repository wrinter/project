...
//设置当前title---------------------------------------------------------------------------------------------------------
thisPi ? $("title").html("布置测试-我的测试") : $("title").html("布置测试-课时练");
...
if(thisPt=="202"||thisPt=="212"){
    var _navLi = "<li><a href='../../model/test/h_publishTest_moList.html?id=" + thisId + "&pt=" + thisPt + "&ac=" + thisActive +  "&kt=" + thisKt + "' class='fl fc65'>模拟套卷</a><i class='spriteImg c_Crumgoico c_Crumgo'></i></li>";
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if(retCode === "0000"){
                ...
                //确认？
                var errorUrl = "<div class='data_Popup data_Error'><div class='in'><img src='../../static/image/test/data_ErrorUrl_bg.jpg' /><div class='btn'><a class='btn_return' style='float: none;margin: 0 auto;' href='javascript:;'>返回</a></div></div></div>";
...
publishWork.prototype = {
    ...
    setPublishWork: function () {
        ...
        var  time = "<div class='timeing'>时间：<span class='timeing_time'>" + Data.testTime +"</span><span>分钟</span>　分数：<span class='timeing_score'>" + Data.score + "</span><span>分</span></div>";
        //添加标题
        $(".exercise_box").prepend(h1 + time );
        //题目
        var ul = SetHtml(Pretreatment(Data),"score",thisSubId);
    ...
    setUrlEdit: function () {
        var Data = this.Data;
        var _href = "";
        thisPi ? _href = "h_publishTest_edit.html?ac=" + thisActive + "&kt=" + thisKt + "&pt=" + thisPt + "&pi=" + thisPi+"&ai="+thisAi+"&st="+thisSt : _href = "h_publishTest_edit.html?id=" + thisId + "&ac=" + thisActive + "&kt=" + thisKt + "&pt=" + thisPt;
        if(Data){

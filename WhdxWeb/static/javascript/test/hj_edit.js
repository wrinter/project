...
function paperInit() {
    ...
    $H.Data.hjPaperData.modify.allLineScoreDef(thisType);//更新数据行分
...
function toPaperinfo(){
    ...
    $(".timeing").append("<span class='test_score_box'>　分数：<span class='test_score'>" + paperInfo.score + "</span>分</span>");//试卷分数
...
function newPaperinfo() {
    ...
    function sCustompapername(data) {
        ...
        $(".timeing").append("<span class='test_score_box'>　分数：<span class='test_score'>0</span>分</span>");//试卷分数
        $H.event.setIsLineNumber({orderLineNumber:true,type:thisType,subject:thisSubject});//首次解析:缓存行号规则

  ...
  export default {
    data () {
      return {
        Videodata: '',
        Views: 0,
        LikeSrc: '../../../../static/teacher/images/chinese/likeico0.png',
    ...
    methods: {
      ...
      LoadVideo (data) {
        ...
        let ThisPlayCode = document.getElementById('PlayCode')
        ThisPlayCode.innerHTML = ''
        ThisPlayCode.appendChild(BaiDuScr)
        this.PlayBack()
      },
      PlayBack () {
        let ScrHtml = ''
      ...
      StartView () {
        let Subdata = {}
        Subdata.resId = UrlSearch('id')
        this.$http.post('/web/common/res/query?' + Math.random(), Subdata, {'emulateJSON': true}).then(() => {})
      }
    }
  }
...
<style>
  ...
  .c_PlayBox{float:left;width: 100%;background: black;}

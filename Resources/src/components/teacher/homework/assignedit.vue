<template>
  <div class="c_Main">
    ...
    <div class="exercise_btn" v-else-if="paperType==116&&status==undefined">
      <div class="exercise_btn_in">
        ...
        <draggable v-model="lines" :move="getdata" @update="datadragEnd" class="work_box work_box_edit">
          <transition-group tag="ul">
            <li class="line" v-for="(line,lindex) in lines" :key="line.lineId">
              <div class="line_title" v-if="line.lineName!='默认题行'">
                <h2>
                  <span class="line_number">{{line.lineNumber}}</span>
                  <span class="line_name">{{line.lineName}}</span>
                  <span class="line_score"></span>
                </h2>
                <span class="line_btn" v-if="line.questions.length>0||line.lineId.indexOf('new')!=-1">
    ...
    <v-setline v-if="lineShow" @updateLine="updateLine" :lines="lines"></v-setline>
  ...
  export default {
    ...
    methods: {
      init () {
        ...
        if (this.paperType === '116' && (status === undefined || status === '')) {
...
//          console.log('move')
      ...
      updateLine (data) {
        for (let i = 0; i < data.length; i++) {
          if (this.subjectId === '03') {
            data[i].lineNumber = this.toRoman(i + 1)
          } else {
      ...
      checkLines () {
        ...
        for (let i = 0; i < this.lines.length; i++) {
          let qus = this.lines[i].questions
          if (this.lines[i].questionType !== '999' && qus.length === 0) {
            flag = false
          }
        }
        return flag

  ...
  <div class="Book_Main">
      <div class="Book_LrcBox">
        <div class="Book_Lrc" id="Book_Lrc" :style="{'top':ThisTop +'px'}">
          <p v-for="(data,index) in ThisIsLyric" :class="{'FC65':data.Class==true}" v-html="data.Content"></p>
        </div>
      </div>
      ...
      GetBook () {
        ...
        this.$http.post('/web/teacher/prepare/voice/en/article?' + Math.random(), Subdata, {'emulateJSON': true}).then((response) => {
          if (response.body.retCode === '0000') {
            this.ThisBookData = response.body.retData
...
<style>
  .Book_Main{float: left;box-sizing: border-box;  width: 100%; height: 530px;  overflow: hidden;  margin-top: 40px;}
  .Book_LrcBox{float: left;width: 100%;height: 450px;margin-bottom: 20px;overflow: hidden;position: relative;}
  .Book_Lrc{float: left;width: 100%;box-sizing: border-box; padding: 0 40px 0 20px;position: absolute;top: 0;left: 0;}
  .Book_Lrc p {width: 100%;  font-size: 16px;  line-height: 30px;  color: #333;float: left;}
